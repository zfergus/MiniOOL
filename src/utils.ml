...
Commands:
 ...
 • e₀(e₁)                  (call procedure evaluated from e₀ with an arguement of e₁ evaluated,
 ...
 • e₀.e₁ = e₂              (assign the value of the expression e₂ to the field e₁ of variable e₀)
 • skip                    (do nothing)
 • {C₀; C₁; ...}           (execute commands sequentially)
 •   while b C             (while b evaluates to true execute the command C)
 • | while b do C
 •   if b C₀ else C₁       (if b evaluates to true execute the command C₀ else execute C₁)
 • | if b then C₀ else C₁
...
Boolean Expressions:
 ...
 •   e₀ == e₁ | e₀ != e₁ | e₀ <> e₁ | e₀ < e₁ | e₀ <= e₁    (comparison operations)
   | e₀ > e₁ | e₀ >= e₁
