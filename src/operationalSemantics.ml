...
let rec string_of_tainted_value v s h =
  match v with
  | Field f -> f
    ...
    Hashtbl.iter
      (fun k v -> if (fst k) = obj then Buffer.add_string buff
            (Printf.sprintf "%s%s: %s"
               ...
               (snd k) (string_of_tainted_value v s h))) h;
...
and string_of_vals s h =
  let buff = Buffer.create ((Hashtbl.length s) * 10) in
  Hashtbl.iter (fun k v ->
      Buffer.add_string buff
        (Printf.sprintf "%s: %s\n" k
           (string_of_tainted_value (Hashtbl.find h (v, "val")) s h))) s;
  Hashtbl.iter
    (fun k v -> if fst k = global_loc then
        Buffer.add_string buff (Printf.sprintf "%s: %s\n" (snd k)
                                  (string_of_tainted_value v s h))) h;
  Buffer.contents buff;;
